...
it('should parse with valid emojis', function (string $actual, Emoji $emoji): void {
    $input = \sprintf('Hello :%s: World', $actual);
    $emoji = \sprintf('<p>Hello %s World</p>', $emoji->value);

    $converter = new CommonMarkConverter();
    $converter->getEnvironment()->addInlineParser(new EmojiParser());

    expect($emoji)->toBe(\trim($converter->convert($input)->getContent()));
})->with(Emoji::dataset());
